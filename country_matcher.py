...
def count_dois(fo):
	print 'Counting DOIs for <%s>.' % fo
	with open('../data_processed/countries/%s' % fo, 'r') as fo:
...
for fo in os.listdir('../data_processed/countries'):
	print 'Reading data from <%s>.' % fo
	doi_counter = count_dois(fo)
	...
	for doi, count in doi_counter.most_common():
		if count > 1:
			...
			with open('../data_processed/subject/%s' % fo, 'a') as csv_out:
